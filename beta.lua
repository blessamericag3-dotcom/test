...
local function performFullReset(chr)
    ...
    removedHairStorage = {}
    if chr then local parent = chr.Parent; chr.Parent = nil; chr.Parent = parent end
end
...
local morpherTab = Window:AddTab("Morpher", "users")
...
makeTabFullWidth(appearanceTab); makeTabFullWidth(morpherTab); makeTabFullWidth(usefulTab)
...
morpherGroup:AddInput("MorphInput", { Text = "User ID / Username", Placeholder = "Enter User ID or Username...", ClearTextOnFocus = true })
local morphStatusLabel = morpherGroup:AddLabel("MorphStatus", { Text = "Status: Idle" })
...
local function getUserIdFromInput(input) local id = tonumber(input); if id then return id end; local s, uId = pcall(Players.GetUserIdFromNameAsync, Players, input); if s and uId then return uId end end

local function morph(userId)
    local success, err = pcall(function()
        performFullReset(Player.Character); local appearance = Players:GetCharacterAppearanceAsync(userId); local char = Player.Character or Player.CharacterAdded:Wait(); task.wait(0.1)
        for _, v in pairs(char:GetChildren()) do if v:IsA("Accessory") or v:IsA("Shirt") or v:IsA("Pants") or v:IsA("CharacterMesh") or v:IsA("BodyColors") or v:IsA("Decal") or v:IsA("ShirtGraphic") then v:Destroy() end end
        scriptedShirtGraphic = nil
        for _, v in pairs(appearance:GetChildren()) do
            if v:IsA("Shirt") or v:IsA("Pants") or v:IsA("BodyColors") or v:IsA("ShirtGraphic") then v:Clone().Parent = char
            elseif v:IsA("Accessory") then if char.Humanoid then local acc = v:Clone(); acc.Parent = workspace; char.Humanoid:AddAccessory(acc) end
            elseif v.Name == "R6" and char.Humanoid.RigType == Enum.HumanoidRigType.R6 then for _, i in ipairs(v:GetChildren()) do if i:IsA("CharacterMesh") then i:Clone().Parent = char end end
            elseif v.Name == "R15" and char.Humanoid.RigType == Enum.HumanoidRigType.R15 then for _, i in ipairs(v:GetChildren()) do if i:IsA("CharacterMesh") then i:Clone().Parent = char end end
            end
        end
        local bodyColors = char:FindFirstChildOfClass("BodyColors")
        if bodyColors then
            applyLimbColor(char, "Head", bodyColors.HeadColor.Color); applyLimbColor(char, "Torso", bodyColors.TorsoColor.Color)
            applyLimbColor(char, "LeftArm", bodyColors.LeftArmColor.Color); applyLimbColor(char, "RightArm", bodyColors.RightArmColor.Color)
            applyLimbColor(char, "LeftLeg", bodyColors.LeftLegColor.Color); applyLimbColor(char, "RightLeg", bodyColors.RightLegColor.Color)
            captureOriginalColors(char, true)
            for limbGroup in pairs(limbMappings) do if Library.Options[limbGroup.."Color"] then Library.Options[limbGroup.."Color"]:SetValueRGB(originalLimbColors[limbGroup]) end end
        end
        if char.Head and char.Head.face then char.Head.face:Destroy() end
        local faceDecal = (appearance:FindFirstChild("Head") and appearance.Head:FindFirstChild("face")) or appearance:FindFirstChild("face")
        if faceDecal and faceDecal:IsA("Decal") and char.Head then faceDecal:Clone().Parent = char.Head
        elseif char.Head then local defaultFace = Instance.new("Decal", char.Head); defaultFace.Name = "face"; defaultFace.Texture = "rbxasset://textures/face.png" end
        local parent = char.Parent; char.Parent = nil; char.Parent = parent; return true
    end)
    return success, err
end

morpherGroup:AddButton("Apply Morph", function()
    local input = Library.Options.MorphInput.Value; if input == "" then morphStatusLabel:SetText("Status: Input is empty.") return end
    morphStatusLabel:SetText("Status: Getting User ID..."); local userId = getUserIdFromInput(input)
    if not userId then morphStatusLabel:SetText("Status: User not found.") return end
    morphStatusLabel:SetText("Status: Morphing..."); local success, err = morph(userId)
    if success then morphStatusLabel:SetText("Status: Morph successful!"); resetAllToggles(); currentMorphUserId = userId else morphStatusLabel:SetText("Status: Morph failed! " .. tostring(err)) end
end)

morpherGroup:AddButton("Reset to Original", function()
    morphStatusLabel:SetText("Status: Resetting..."); local success, err = morph(Player.UserId)
    if success then morphStatusLabel:SetText("Status: Reset successful!"); resetAllToggles(); currentMorphUserId = nil else morphStatusLabel:SetText("Status: Reset failed! " .. tostring(err)) end
end)

local popularUsers = {1, 261, 156, 3, 7, 17, 31, 41, 69, 123, 196, 256, 333, 696, 1560, 2600, 3000, 15600, 196000, 261000}
morpherGroup:AddButton("Random Morph", function()
    local randomId = popularUsers[math.random(1, #popularUsers)]; morphStatusLabel:SetText("Status: Morphing to random user " .. tostring(randomId) .. "...")
    local success, err = morph(randomId)
    if success then morphStatusLabel:SetText("Status: Morph successful!"); resetAllToggles(); currentMorphUserId = randomId else morphStatusLabel:SetText("Status: Morph failed! " .. tostring(err)) end
end)
...
local function fullCharacterReset()
    performFullReset(Player.Character); resetAllToggles(); resetLimbColors()
end
...
if ThemeManager and SaveManager then
    ...
    SaveManager:IgnoreThemeSettings(); SaveManager:SetIgnoreIndexes({ "ToggleUIKeybind", "MorphInput" })
    ...
    function SaveManager:Save(name)
        ...
        if success then
            ...
            data.morphedUserId = currentMorphUserId
    ...
    function SaveManager:Load(name)
        morph(Player.UserId) -- Always reset to original character before loading anything.
        task.wait(0.5)

        local path = self.Folder .. "/settings/" .. name .. ".json"
        if not isfile(path) then return oldLoadFunc(self, name) end
        local data = game:GetService("HttpService"):JSONDecode(readfile(path))
        
        local success, err = oldLoadFunc(self, name)
        if not success then return success, err end
        
        task.wait(0.2)
        if data.morphedUserId then
            morph(data.morphedUserId)
            currentMorphUserId = data.morphedUserId
        else
